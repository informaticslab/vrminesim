/**
 * Based on MobileHUDExt:
 *   Copyright 1998-2011 Epic Games, Inc. All Rights Reserved.
 */
class MineSimHUD extends MobileHUD;
...
/** Scope Circle */
var	bool		bShowScope;
var Texture2D	ScopeCircleTexture;
var	float		ScopeCircleWidth;
var	float		ScopeCircleHeight;

/** Settings Button */
var Texture2D	SettingsButton;
var	float		SettingsButtonWidth;
var	float		SettingsButtonHeight;

/** Overridden to store the Settings Button Dimensions(no need to grab them every frame) */
simulated function PostBeginPlay()
{
	super.PostBeginPlay();
	
	ScopeCircleWidth = ScopeCircleTexture.GetSurfaceWidth();
	ScopeCircleHeight = ScopeCircleTexture.GetSurfaceHeight();

	SettingsButtonWidth = SettingsButton.GetSurfaceWidth();
	SettingsButtonHeight = SettingsButton.GetSurfaceHeight();
}
...
function PostRender()
{
	...
	if (bShowScope)
	{
		Canvas.SetPos((Canvas.SizeX - ScopeCircleWidth) * 0.5, (Canvas.SizeY - ScopeCircleHeight) * 0.5);
		Canvas.SetDrawColor(255, 255, 255, 200);
		Canvas.DrawTile(ScopeCircleTexture, ScopeCircleWidth, ScopeCircleHeight, 0.0, 0.0, ScopeCircleWidth, ScopeCircleHeight);
	}

	// Draw the Settings Button
	Canvas.SetPos((Canvas.SizeX - SettingsButtonWidth) * 0.5, Canvas.SizeY - SettingsButtonHeight * 1.25);
	Canvas.SetDrawColor(255, 255, 255, 200);
	Canvas.DrawTile(SettingsButton, SettingsButtonWidth, SettingsButtonHeight, 0.0, 0.0, SettingsButtonWidth, SettingsButtonHeight);

	super.PostRender();	

/*	Removed to reduce log spam(our PlayerController class doesn't subclass CastlePC)
...
function DrawMobileZone_Slider(MobileInputZone Zone)
{
	...
	Canvas.SetPos(Zone.CurrentLocation.X, Zone.CurrentLocation.Y - 16);
...
defaultproperties
{
	...
	ButtonFont = MultiFont'CastleFonts.Positec'
	...
	ScopeCircleTexture=Texture2D'MineSimHUD.Circle'

	SettingsButton=Texture2D'MineSimHUD.SettingsButton'
}
